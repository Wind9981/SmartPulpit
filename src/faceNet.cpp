...
string FaceNetClassifier::featureMatching(cv::Mat &image) {
    string personName;
    double minDistance;
    for(int i = 0; i < (m_embeddings.size()/128); i++) {
        minDistance = 10.* m_knownPersonThresh;
        ...
        if (minDistance <= m_knownPersonThresh) {
            cout << "Name: " << m_knownFaces[winner].className << endl;
            ...
            personName = m_knownFaces[winner].className;
        }
        else if (minDistance > m_knownPersonThresh || winner == -1){
            cout << "Name: New Person"  << endl;
